				...
				<div class="row pb-3">
					...
					<c:if test="${productsPaginate.size()>0 }">
						<div class="col-lg-4 col-md-6 col-sm-12 pb-1">
							<c:forEach var="p" items="${proAllPaginate}" varStatus="loop">
								...
								<c:if
									test="${ (loop.index + 1) % 2 == 0 || (loop.index + 1)  == proAllPaginate.size() }">
									</ul>
						</div>
						<c:if test="${ (loop.index + 1) < proAllPaginate.size() }">
							<div class="col-lg-4 col-md-6 col-sm-12 pb-1">
						</c:if>
					</c:if>
					</c:forEach>
					...
					<div class="col-12 pb-1">
						<nav aria-label="Page navigation">
							<ul class="pagination justify-content-center mb-3">
								...
								<c:forEach var="p" begin="1" end="${proInfo.totalPage}"
									varStatus="loop">
									<c:if test="${(loop.index ) == proInfo.currentPage}">
										<li class="page-item active"><a class="page-link"
											href="<c:url value="/Home/productList/${loop.index}" />">${loop.index}</a></li>
									</c:if>
									<c:if test="${ (loop.index ) != proInfo.currentPage}">
